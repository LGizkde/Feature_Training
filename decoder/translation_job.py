...
import hypergraph
from hypergraph import Hypergraph, Path
import logger
import pickle
...
class TranslationJob(object):
    ...
    def run(self, feature = None):
        ...
        for grammar in self.grammars:
            grammar.update_features(feature)
            #grammar.reset()
            #grammar.initialize()

        list1 = self.features.get_features()
        print ('FEATURE:' + str(list1))
        ...
        if len(rules) > 0:
          ...
          print ('Rule Features:' + str(self.grammars[0].features))
          #@LGizkde
          local_gr.build(rules, self.features)
        ...
        if item is None:
            ...
            #self.flog.write('%s\n' % (self.kbest[0].edge))
            #self.flog.write('%s\n' % (self.kbest[0].edge.fcosts))
            #self.flog.write('%s\n' % (self.kbest[0].get_fcosts_list()))
            ...
            self.flog.write(self.kbest[0].translation.to_amr_format()[0] + '\n')
            #else:
        ...
        #@LGizkde
        #cost_list = [a - b for a, b in zip(self.kbest[0].get_fcosts_list(), self.kbest[0].edge.fcosts)]
        return self.kbest[0].get_fcosts_list()
