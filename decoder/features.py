...
class Features(object):
    ...
    #@LGizkde
    def update_weights(self, costs, std_costs, tr):
        delta_costs = [a - b for (a, b) in zip(costs, std_costs)]
        self.stateless = [(a, b + c * tr) for ((a, b), c) in zip(self.stateless, delta_costs)]
    ...
    #@LGizkde
    def rule_costs(self, rule):
        """computing every feature cost for a rule, this uses only stateless features """
        cost_list = []
        for feature, weight in self.stateless:
            cost_list.append(feature.weight(rule))
    ...
    #@LGizkde
    def score_list(self, costs):
        s = 0
        for ((f, w), c) in zip(self.stateless, costs):
            s += w * c
        return s
    ...
    def __str__(self):
        s = 'Stateful:'
        for f, w in self.stateful:
            s += str(f) + ': ' + str(w) + ', '
        s += '\nSteteless:'
        for f, w in self.stateless:
            s += str(f) + ': ' + str(w) + ', '
        return s
