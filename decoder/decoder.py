...
gflags.DEFINE_string(
    'train',
    None,
    'Train file for training. Seperated by blank line')
gflags.DEFINE_float(
    'tr',
    0.1,
    'Training Rate')
...
def get_train_jobs(cnt = -1):
    ...
    for line in f:
        if len(line.strip()) <= 1:
            jobs.append(rule_string)
            rule_string = []
            cnt -= 1
            if not cnt:
                return jobs
        else:
            rule_string.append(line.strip())
...
def single_worker_decode():
    ...
    totaltime = 0
    joblogger = JobLogger()
    features = Features(FLAGS.features)
    while jobs:
        # finished jobs need to be discarded because jobs save the hypergraph
        job = jobs.pop(0)
        cost_list = job.run(features)
        gold_rules = train.pop(0)
        gold_cost_list = [0] * len(cost_list)
        for rule_str in gold_rules:
            rule = Rule()
            rule.fromstr(rule_str)
            gold_cost_list = [a + b for (a, b) in zip(gold_cost_list, features.rule_costs(rule))]
       
        print (str(cost_list), features.score_list(cost_list))
        print (str(gold_cost_list), features.score_list(gold_cost_list))
        features.update_weights(cost_list, gold_cost_list, FLAGS.tr)
        print (str(features))
        totaltime += job.time
